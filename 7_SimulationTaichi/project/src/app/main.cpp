...
void add_object_circle(Vec center, real radius, int c, int num = 1000, Vec velocity = Vec(0.0_f))
...
void add_object(Vec center, int c, Vec velocity = Vec(0.0_f), int ptype = 0) {
  // Randomly sample 1000 particles in the square
  for (int i = 0; i < 1000; i++) {
    particles.push_back(Particle((Vec::rand()*2.0f-Vec(1))*0.04f + center, c, velocity, ptype));
  }
}
void add_jet() {
   add_object_rectangle(Vec(0.05, 0.5), Vec(0.07, 0.51), 0x87CEFA, 10, Vec(7.0, 0.0));
   add_object_rectangle(Vec(0.5, 0.7), Vec(0.52, 0.71), 0x87CEFA, 10, Vec(0.0, -10.0));
}
...
int main() {
  ...
  //add_object(Vec(0.55,0.45), 0xED553B);
   add_object(Vec(0.4,0.75), 0xF2B134, Vec(0, -20),1);
 // add_object_circle(Vec(0.45, 0.65), 0.06, 0xFFFAFA, 1000, Vec(0, -20));
 // add_object_triangle(Vec(0.7, 0.5), Vec(0.85, 0.65), 0xED553B, 1000, Vec(0, -20));
  //add_object(Vec(0.55,0.85), 0x068587);
  //add_object_rectangle(Vec(0.05, 0.04), Vec(0.95, 0.34), 0x068587, 20000, Vec(0.0, 0.0), 0);
    add_object_rectangle(Vec(0.35, 0.04), Vec(0.65, 0.34), 0x068587, 5000, Vec(0.0, 0.0), 1);
  int frame = 0;
