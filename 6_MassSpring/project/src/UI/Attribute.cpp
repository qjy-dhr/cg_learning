...
void Attribute::ComponentVisitor::ImplVisit(Ptr<CmptSimulate> simulate) {
	...
	auto grid = GetGrid(item);
	grid->AddText("- Gravity");
	grid->AddEditVal("X", simulate->GetGravity()[0], 0.01);
	grid->AddEditVal("Y", simulate->GetGravity()[1], 0.01);
	grid->AddEditVal("Z", simulate->GetGravity()[2], 0.01);
	...
	grid->AddButton("set y max x boundary fix", [simulate]() {
		simulate->SetUpLRFix();
		});
	...
	vector<string> method_choice_ = { "Implicit Euler","Projective Dynamic" };
	grid->AddComboBox(shape, "Choose Simulate Method", method_choice_, [=](const string& istem) {
		if (shape->currentIndex() == 0) {
			cout << "Select method : " << method_choice_[shape->currentIndex()] << endl;
...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	//选择边界形状
	QComboBox* shape = new QComboBox();
	vector<string> shape_choice_ = { "Circle","Square" };
	grid->AddComboBox(shape, "ParamaterizeShape", shape_choice_, [=](const string& istem) {
		cout << "Select shape : " << shape_choice_[shape->currentIndex()] << endl;
		});

	//选择权重
	QComboBox* weight = new QComboBox();
	vector<string> weight_choice_ = { "Uniform","Cotangent" };
	grid->AddComboBox(weight, "ParamaterizeWeight", weight_choice_, [=](const string& item) {
		cout << "Select weight : " << weight_choice_[weight->currentIndex()] << endl;
		});

	grid->AddButton("Paramaterize", [mesh, pOGLW = attr->pOGLW,shape, weight]() {
		printf("[Paramaterize] start\n");
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->setShape(shape->currentIndex());
		paramaterize->setWeight(weight->currentIndex());
		if (paramaterize->Run())
			printf("[Paramaterize] success\n");
		else
			printf("[Paramaterize] fail\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize Mesh", [mesh, pOGLW = attr->pOGLW, shape, weight]() {
		printf("[Paramaterize Mesh] start\n");
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->setShape(shape->currentIndex());
		paramaterize->setWeight(weight->currentIndex());
		if (paramaterize->GetMesh())
			printf("[Paramaterize Mesh] success\n");
		else
			printf("[Paramaterize Mesh] fail\n");
		pOGLW->DirtyVAO(mesh);
	});
	...
	grid->AddButton("ASAP Mesh", [mesh, pOGLW = attr->pOGLW]() {
		printf("[ASAP Mesh] start\n");
		auto Asap = ASAP::New(mesh);
		if (Asap->GetMesh())
			printf("[ASAP Mesh] success\n");
		else
			printf("[ASAP Mesh] fail\n");
		pOGLW->DirtyVAO(mesh);
	});

	//添加ARAP按钮
	int *iteration_time = new int(1);
	grid->AddEditVal({ "ARAP Iteration times" }, 1,1,100, [=](const int& val){
		cout << "Select iteration times : " << val<< endl;
		*iteration_time = val;
		});

	grid->AddButton("ARAP", [mesh, pOGLW = attr->pOGLW, iteration_time]() {
		printf("[ARAP] start\n");
		auto Arap = ARAP::New(mesh);
		Arap->SetIteration(*iteration_time);
		if (Arap->Run())
			printf("[ARAP] success\n");
		else
			printf("[ARAP] fail\n");
	...
	grid->AddButton("ARAP Mesh", [mesh, pOGLW = attr->pOGLW, iteration_time]() {
		printf("[ARAP Mesh] start\n");
		auto Arap = ARAP::New(mesh);
		Arap->SetIteration(*iteration_time);
		if (Arap->getMesh())
			printf("[ARAP Mesh] success\n");
		else
			printf("[ARAP Mesh] fail\n");
		pOGLW->DirtyVAO(mesh);
	});
